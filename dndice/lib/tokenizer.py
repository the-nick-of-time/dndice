...
class State:
    def __init__(self, expression: str, i: int):
        ...
        # Every character that could be part of an operator
        self.ops = set(''.join(OPERATORS)) - set('p')

    def run(self) -> typing.Tuple[typing.Union[Token, None], 'State']:
        raise NotImplementedError

    def is_digit(self, char: str) -> bool:
        return char in self.digits

    def is_op(self, char: str) -> bool:
        return char in self.ops
    ...
    @staticmethod
    def is_list_start(char: str):
        return char == '['

    def is_unrecognized(self, char: str):
        return not (self.is_digit(char)
                    or self.is_op(char)
                    or self.is_whitespace(char)
                    or self.is_list_start(char))
    ...
    def run(self) -> typing.Tuple[typing.Union[Token, None], 'State']:
        while self.index < len(self.expression):
            ...
            if self.is_unrecognized(char):
                raise ParseError("Unrecognized character detected", self.index, self.expression)
            if self.is_digit(char):
                return None, BuildNumber(self.expression, self.index)
            if self.is_op(char):
                return None, BuildOperator(self.expression, self.index)
            # Whitespace just continues being Blank
            self.index += 1
        # Reached end of string
        return None, Finish()


class BuildNumber(State):
    def run(self) -> typing.Tuple[typing.Union[Token, None], 'State']:
        num = []
        while self.index < len(self.expression):
            char = self.expression[self.index]
            if self.is_unrecognized(char):
                raise ParseError("Unrecognized character detected", self.index, self.expression)
            if self.is_op(char):
                return self._collect(num), BuildOperator(self.expression, self.index)
            if self.is_whitespace(char):
                return self._collect(num), Blank(self.expression, self.index)
            num.append(char)
            self.index += 1
        return self._collect(num), Finish()

    @staticmethod
    def _collect(num: typing.List[str]) -> int:
        return int(''.join(num))


class BuildOperator(State):
    def run(self) -> typing.Tuple[typing.Union[Token, None], 'State']:
        op = []
        while self.index < len(self.expression):
            char = self.expression[self.index]
            if self.is_unrecognized(char):
                raise ParseError("Unrecognized character detected", self.index, self.expression)
            if self.is_whitespace(char):
                return self._collect(op), Blank(self.expression, self.index)
            op.append(char)
            self.index += 1
        return self._collect(op), Finish()

    def _collect(self, op: typing.List[str]) -> typing.Union[str, Operator]:
        return _string_to_operator(''.join(op), self.index, self.expression)


class BuildList(State):
    def is_separator(self, char: str):
        return char.isspace() or char == ',' or self.is_list_start(char)

    def is_number(self, char: str):
        return self.is_digit(char) or char == '.'

    @staticmethod
    def is_list_end(char: str):
        return char == ']'

    def run(self) -> typing.Tuple[typing.Union[Token, None], 'State']:
        state = ListSeparator(self.expression, self.index)
        collection = []  # type: List[float]
        while not isinstance(state, Blank):
            number, state = state.run()
            if number is not None:
                collection.append(number)
        return tuple(collection), Blank(self.expression, state.index)


class ListSeparator(BuildList):
    def run(self) -> typing.Tuple[typing.Union[Token, None], 'State']:
        while self.index < len(self.expression):
            char = self.expression[self.index]
            if self.is_number(char):
                return None, ListItem(self.expression, self.index)
            self.index += 1


class ListItem(BuildList):
    def run(self) -> typing.Tuple[typing.Union[Token, None, float], 'State']:
        num = []
        while self.index < len(self.expression):
            char = self.expression[self.index]
            if self.is_separator(char):
                return self._collect(num), ListSeparator(self.expression, self.index)
            self.index += 1

    @staticmethod
    def _collect(agg: typing.List[str]):
...
class Tokenizer:
    ...
    def __call__(self) -> typing.Iterable[Token]:
        state = Blank(self.expr, 0)
        while not isinstance(state, Finish):
